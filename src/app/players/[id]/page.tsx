    ...
    <SectionCard title="Player Details" icon={<ShieldCheck className="h-4 w-4 text-primary" />}>
        <div className="space-y-4">
          <div className="flex flex-col items-center rounded-2xl bg-gradient-to-b from-primary/10 via-primary/[0.04] to-transparent p-4 text-center ring-1 ring-primary/15">
            <div className="relative flex h-20 w-20 items-center justify-center overflow-hidden rounded-3xl bg-background/70 ring-2 ring-primary/35">{player.avatar_url ? <img src={player.avatar_url} alt={player.name} className="h-full w-full object-cover" /> : <span className="text-xl font-bold text-foreground">{getInitials(player.name)}</span>}{teamLogo ? <span className="absolute bottom-1 right-1 flex h-7 w-7 items-center justify-center overflow-hidden rounded-full bg-zinc-950 ring-2 ring-card"><img src={teamLogo} alt={`${player.team ?? ""} logo`} className="h-full w-full object-contain p-1" /></span> : null}</div>
